...
data <- data %>% select(prname, date, numtoday)  %>% filter(date > (max(date)) - 40) %>%
...
for (i in data$prname){
  ...
  # rolling <- data %>% filter(prname == i) %>% arrange(desc(date)) %>% mutate(rolling = zoo::rollmean(numtoday, k = 7, fill = NA)) 
...
colors <- c("gray76", "tomato", "green1")
# New facet label names for dose variable
var.labs <- c("cases_7" = "NEW CASES OVER 7 DAY PERIOD / 100K POPULATION",
              "change" = "CHANGE IN RATE FROM PREVIOUS WEEK")
...
idx <- match(m_df$variable, names(var.labs))
m_df$var.labs2 <- var.labs[idx]
m_df$var.labs2 <- factor(m_df$var.labs2, levels = c("NEW CASES OVER 7 DAY PERIOD / 100K POPULATION", "CHANGE IN RATE FROM PREVIOUS WEEK"))


pfw <- ggplot(data = m_df, aes(x = Province, y = value, group= variable, fill = fill)) +
  geom_bar(stat = "identity", data = m_df) + scale_fill_manual(values=colors)+
  geom_text(aes(label = round(value,1)), colour = "black", y = 3, hjust = 0) +
  facet_grid(~ var.labs2, labeller = labeller(var.labs2 = label_wrap_gen(25)), scales = "free", space = "free_x") +
  guides(fill = FALSE) +  # to remove the legend
  theme_bw() + coord_flip() + labs(x="", y="") + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) +
  theme(strip.text = element_text(size=10)) + theme(strip.background =element_rect(fill="gray96"))+
  theme(axis.text.y = element_text(size = 11, colour = "black")) #+ scale_y_continuous(expand = c(0.1, 4))
                                                                        # + ylim((min(table_values$change)*1), (max(table_values$cases_7)*1.25))

# change scales...change number position on negative side. 

# library(grid)
# gt = ggplot_gtable(ggplot_build(pfw))
# gt$widths[7] = 1*gt$widths[7]
# grid.draw(gt)
# library(gtable)
...
```{r,echo=FALSE, fig.height=4, fig.width=9, fig.fullwidth = FALSE, message=FALSE, warning=FALSE}
...
for (i in unique(HZ_data$HR_UID)){
  ...
  change <- cases_7 - cases_previous_week
  mutate_cases <- HZ_data %>% filter(HR_UID == i) %>% arrange(desc(Last_Updated)) %>% mutate(new_cases = CaseCount - lead(CaseCount)) %>% mutate(rolling = rollsum(new_cases, k = 7, fill = NA, align = "left")) %>% arrange(Last_Updated)
  #population <- HZ_data %>% filter(HR_UID == i) %>% select(TotalPop2019) %>% first()/100000
  #################
  # PTs <- data$prname[1:13]
  # ii <- which(PTs == i)
  mutate_cases$rolling <- gsub(",","",mutate_cases$rolling)
  
  HZ_table_values[ii,1:6] <- data.frame(cases_14$Province[1], i, cases_7, change, max(cases_14$Last_Updated), (x = paste(mutate_cases$rolling, collapse = ',')))# list(mutate_cases$Last_Updated))#, list(mutate_cases$rolling))
  
  #sparkline[nrow(sparkline)+1:nrow(mutate_cases),1:3] <- data.frame(mutate_cases$HR_UID, mutate_cases$Last_Updated, mutate_cases$rolling)
...
### will need to factor in population to rolling averages above.
...
names(HZ_table_values2)[c(2,5,6)] <- c("Province", "Last_Updated", "Rolling")

#### adjust rolling sums to include population denominator
new_rolling <- c()
maxs <- c()
for (i in 1:nrow(HZ_table_values2)){
  trendline <- HZ_table_values2[i,c(6,8)]
  #trendline <- HZ_table_values2 %>% slice(i) #%>% select(Rolling, TotalPop2019)#
  x <- unlist(strsplit(trendline$Rolling, split = ","))
  x <- as.numeric(x)/ trendline$TotalPop2019
  x <- round(x, digits = 1)
  maxs[i] <- x[15:length(x)] %>% na.omit() %>% max() #max(x[!is.na(x[15:length(x)])])
  x <- paste(x, collapse = ',')
  new_rolling[i] <- x
}

HZ_table_values2$Rolling <- new_rolling

# sparkline(as.numeric(unlist(strsplit(HZ_table_values2$sparkline[[index]], split = ","))), width = 240)
...
QC_names <- HZ_table_values2$ENGNAME[HZ_table_values2$Province == "QC"]
...
HZ_table_values2$ENGNAME[HZ_table_values2$Province == "QC"] <- new_QC_names[match(HZ_table_values2$ENGNAME[HZ_table_values2$Province == "QC"], QC_names)]
...
### format names
HZ_table_values2$ENGNAME[HZ_table_values2$ENGNAME == "Southern Healthâ€”SantÃ© Sud"] <- "Southern Health"
########################### The table

#colnames(HZ_table_values2)[6] <- "sparkline"
HZ_table_values2$Rolling2 <- HZ_table_values2$Rolling
HZ_table_values2 <- HZ_table_values2 %>% select(PT, ENGNAME, cases_7, change, Rolling, Rolling2)
...
table <- reactable(HZ_table_values2, defaultSortOrder = 'desc',
  ...
  compact = TRUE, filterable = TRUE, pagination = FALSE,  defaultColDef = colDef(
    ...
    Rolling = colDef(
    name = "Case rate over past month",
    maxWidth = 200,
    align = "center",
    cell = function(value, index) {
    sparkline(na.omit(as.numeric(unlist(strsplit(HZ_table_values2$Rolling[[index]], split = ",")))), width = "100%")
      }),
    Rolling2 = colDef(
    name = "Fixed Scale",
    maxWidth = 200,
    align = "center",
    cell = function(value, index) {
    sparkline(na.omit(as.numeric(unlist(strsplit(HZ_table_values2$Rolling[[index]], split = ",")))), width = "100%",  fillColor = 'pink', lineColor = 'red', chartRangeMax = max(maxs))
      })
      ))

#y <- list(as.numeric(unlist(strsplit(as.character(HZ_table_values2$Rolling), split = ","))))

#y <- strsplit(as.character(HZ_table_values2$Rolling[1]), split = ",")
    ...
    div(
      ...
      p(
        paste0("Updated on ", format(Sys.Date(),"%B "),
          ...
          strftime(Sys.time(), format = "%H:%M"), " ET.", " Click on header title to sort table, or search for a region using the search tab. A rolling 7-day case rate is displayed on the line graphs. Hover over the graph to see the date and the case rate over the previous 7 days."
        )
      ),
    ),
    table)
